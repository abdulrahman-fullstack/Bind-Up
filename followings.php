	...
	<div class="spo">
		<div class="side-pane">
			...
			<div class="followscc cont-changer cur">
				<span class="active"></span><a href="followings.php">Followings</a>
			</div>
		...
		<div class="cont-block">
			<div class="content">
				...
				<?php
					...
					if($followers = DB::query("SELECT * FROM followers WHERE follower_id=:logged_user_id ",array(':logged_user_id'=>$logged_user['id']))){

						foreach($followers as $follower):
							
							$people = DB::query("SELECT * FROM users WHERE id=:user_id ORDER BY id DESC",array(':user_id'=>$follower['user_id']))[0];
							
							$follow = DB::query("SELECT * FROM followers 
								WHERE user_id=:user_id AND follower_id=:follower_id",
								array(':user_id'=>$follower['user_id'],':follower_id'=>$logged_user['id'])) ? 'Unfollow' : 'Follow' ;
							...
							echo '<div class="flwr">
										...
										<div class="flw-cont">
											<input type="button" value="'.$follow.'" class="'.$follow.' follow-btn" data-uid="'.$follower['user_id'].'">
										</div>
									</div>';
